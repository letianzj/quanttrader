...
class DoubleMovingAverageCrossStrategy(StrategyBase):
    ...
    def __init__(self):
        ...
        ).seconds  # start_time is always positive even if start_time - end_time
        # 6.5*60*60 = 23400
    ...
    def on_tick(self, tick_event):
        ...
        if seconds == self.df_bar_idx:  # same bar
            self.df_bar["High"].iloc[seconds] = max(
                self.df_bar["High"].iloc[seconds], k.price
            )
            self.df_bar["Low"].iloc[seconds] = min(
        ...
        if df1.shape[0] < self.n_slow_ma:
            _logger.info(
                f"DoubleMovingAverageCrossStrategy wait for enough bars, {df1.shape[0]} / {self.n_slow_ma}"
            )
            return
        ...
        if ma_fast > ma_slow:
            if self.current_pos <= 0:
                ...
                _logger.info(
                    f"DoubleMovingAverageCrossStrategy long order placed, on tick time {k.timestamp}, current size {self.current_pos}, order size {o.order_size}, ma_fast {ma_fast}, ma_slow {ma_slow}"
                )
                self.current_pos = 1
                self.place_order(o)
            ...
            if self.current_pos >= 0:
                ...
                o.order_size = -1 - self.current_pos
                _logger.info(
                    f"DoubleMovingAverageCrossStrategy short order placed, on tick time {k.timestamp}, current size {self.current_pos}, order size {o.order_size}, ma_fast {ma_fast}, ma_slow {ma_slow}"
                )
                self.current_pos = -1
                self.place_order(o)
