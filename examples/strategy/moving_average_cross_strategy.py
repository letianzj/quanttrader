...
class MovingAverageCrossStrategy(StrategyBase):
    ...
    def on_tick(self, tick_event):
        ...
        if k.tick_type == TickType.ASK:
            ...
            if (
                self.ema == -1
            ):  # intialize ema; alternative is to use yesterday close in __init__
            ...
            if k.price > self.ema:  # (flip to) long
                ...
                if len(standing_oids) > 0:
                    _logger.info(
                        f"MovingAverageCrossStrategy standing orders: {','.join(map(str, standing_oids))}"
                    )
                ...
                for oid in standing_oids:
                    if self._order_manager.retrieve_order(oid).order_size > 0:
                        _logger.info(
                            f"MovingAverageCrossStrategy long order already in place. ema {self.ema}, last {k.price}, bid {self.last_bid}"
                        )
                        return
                    elif self._order_manager.retrieve_order(oid).order_size < 0:
                        _logger.info(
                            f"MovingAverageCrossStrategy cancel existing short. ema {self.ema}, last {k.price}, bid {self.last_bid}"
                        )
                ...
                _logger.info(
                    f"MovingAverageCrossStrategy long order placed, current size {current_pos}, order size {o.order_size}. ema {self.ema}, last {k.price}, bid {self.last_bid}"
                )
                self.place_order(o)
            else:  # (flip to) short
                ...
                for oid in standing_oids:
                    if self._order_manager.retrieve_order(oid).order_size < 0:
                        _logger.info(
                            f"MovingAverageCrossStrategy short order already in place. ema {self.ema}, last {k.price}, bid {self.last_bid}"
                        )
                        return
                    elif self._order_manager.retrieve_order(oid).order_size > 0:
                        _logger.info(
                            f"MovingAverageCrossStrategy cancel existing long. ema {self.ema}, last {k.price}, bid {self.last_bid}"
                        )
                        self.cancel_order(oid)
                ...
                o.order_size = -1 - current_pos
                _logger.info(
                    f"MovingAverageCrossStrategy short order placed, current size {current_pos}, order size {o.order_size}, ema {self.ema}, last {k.price}, ask {self.last_ask}"
                )
                self.place_order(o)
